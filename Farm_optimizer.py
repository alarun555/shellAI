# -*- coding: utf-8 -*-
"""
Created on Tue Sep 15 14:21:44 2020
...
"Particle Swarm Optimization Method Begins"
...
class Particle():
    def __init__(self):
        positionInit=np.zeros(100)
        for i in range(0,100):
            positionInit[i]=(-1) ** (bool(random.getrandbits(1))) * random.random()*3950
...
class Space():
    ...
    def __init__(self, target, target_error, n_particles):
        ...
        gbestInit=np.zeros(100)
        for i in range(0,100):
            gbestInit[i]=random.random()*3950
        self.gbest_position = gbestInit
    ...
    def fitness(self, particle):
        "Converting particle position to XY"
        particlePositionXY=np.zeros((50,2))
        for i in range(0,100,2):
            j=0;
            particlePositionXY[j][0]=particle.position[i]
            particlePositionXY[j][1]=particle.position[i+1]
            j=j+1;
            
        return totalAEP(turb_diam, particlePositionXY, power_curve, wind_inst_freq)
    ...
    def set_pbest(self):
        for particle in self.particles:
            ...
            "1.Checking and correcting perimeter contraints"
            for i in range(0,100):
                if(particle.position[i]<50):
                    particle.position[i]=50
                if(particle.position[i]>3950):
                    particle.position[i]=3950
                    
            particleXYarray=np.zeros((50,2))
            distCkArray=np.zeros((50,50))
            
            "2.Creating XY array for all turbines for the current instance"
            
            j=0;
            for i in range(0,100,2):
                
                particleXYarray[j][0]=particle.position[i]
                particleXYarray[j][1]=particle.position[i+1]
                j=j+1;
                
                
            "3.Creating DistanceCheck Matrix corresponding to absolute center to center dist"
            
            for i in range(0,50):
                for j in range(0,50):
                    distCkArray[i][j]=math.sqrt((particleXYarray[i][0]-particleXYarray[j][0])**2+(particleXYarray[i][1]-particleXYarray[j][1])**2)
                    
            
            "4. Correcting close turbines under 400m by directly incrementing both"
            "x and y coordinates (possible improvement by taking theta and incrementing"
            "sin and cos component of theta values)"

            for k in range(0,50):
                for i in range(1,50):
                    for j in range(0,i-1):
                        if(distCkArray[i][j]<400):
                            
                            while(math.sqrt((particleXYarray[i][0]-particleXYarray[j][0])**2+(particleXYarray[i][1]-particleXYarray[j][1])**2)<400):
                                particleXYarray[j][0]=particleXYarray[j][0]+1;
                                particleXYarray[j][1]=particleXYarray[j][1]+1;
                            
                            
            "5. Return values to particle.postion"
            j=0;
            for i in range(0,100,2):
                
                particle.position[i]=particleXYarray[j][0]
                particle.position[i+1]=particleXYarray[j][1]
                j=j+1;
            
            "End of contrains handling"
            
            if(particle.pbest_value < fitness_cadidate):
                particle.pbest_value = fitness_cadidate
                particle.pbest_position = particle.position
            
            "NOTE: THE OUTPUT IS IN FORM [x1,y1,x2,y2,x3,y3...] and so on corresponding"
            "to a continuous list of turbine coordinates"
...
"Converting GBest to XY form (table X and Y columns)"

gbestFinal=np.zeros((50,2)) 
q=0;
for i in range(0,100,2):
                
    gbestFinal[q][0]=search_space.gbest_position[i]
    gbestFinal[q][1]=search_space.gbest_position[i+1]
    q=q+1;    

print("The best solution is: ", gbestFinal, " in n_iterations: ", iteration)
np.savetxt("Soln.csv", gbestFinal, delimiter=",",header="x,y")
         
print(totalAEP(turb_diam, gbestFinal, power_curve, wind_inst_freq))